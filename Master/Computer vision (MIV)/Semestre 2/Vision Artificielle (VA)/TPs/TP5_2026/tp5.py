...
query = faces["s20/9.pgm"].reshape(1,-1)
query_weight = eigenfaces @ (query - pca.mean_).T

print(eigenfaces.shape, weights.shape, query_weight.shape)

euclidean_distance = np.linalg.norm(weights - query_weight, axis=0)
best_match = np.argmin(euclidean_distance)

#Egallité ou presque de l'image et la somme pondérée des composantes principales 
image= (pca.mean_.T).reshape(1,-1)+(query_weight).T @ eigenfaces
print(((pca.mean_.T).reshape(1,-1)).shape)
...
fig, axes = plt.subplots(1,2,sharex=True,sharey=True,figsize=(8,6))
axes[0].imshow(image.reshape(faceshape), cmap="gray")
axes[0].set_title("Query_reconstructed")
plt.show()