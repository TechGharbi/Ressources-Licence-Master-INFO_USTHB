...
void smallest(int *least, int *second, int lasttree, foret forest)
{
 ...
 /// huffman
void Huffman(foret forest, int lastnode, int lasttree, tree arbre)  /*  cr�er l'arbre  */
{
	int i, j;
	/* test s'il ya des arbres dans foret pour cr�er un nouveau noeud */
	while(lasttree > 1)
	{
		smallest(&i, &j, lasttree, forest);
		/* chercher les indices des arbres de frequence min de la foret

				/* cr�er un nouveau noeud dont le sous arbre gauche sera
		...
		/* j sera �cras� par le dernier arbre de la foret */
...
do
	...
	case 6: printf("\nla processus quantification inverse :\n");
				 ...
				 e=fopen("cos2.txt","r");
			  printf("\nvoici la transform�e dct inverse : \n\n");
				for(u=0;u<8;u++)
			  {
			  for(v=0;v<8;v++)
				  {
